...
public class AccountMapper {
    ...
    public static Account fromCursor (Cursor cursor, Context context) {
        int id = cursor.getInt(cursor.getColumnIndexOrThrow(AccountDatabaseHelper.COLUMN_ACCOUNT_ID));
        String username = cursor.getString(cursor.getColumnIndexOrThrow(AccountDatabaseHelper.COLUMN_USERNAME));
        String password = cursor.getString(cursor.getColumnIndexOrThrow(AccountDatabaseHelper.COLUMN_PASSWORD));
        String email = cursor.getString(cursor.getColumnIndexOrThrow(AccountDatabaseHelper.COLUMN_EMAIL));
        boolean role = cursor.getInt(cursor.getColumnIndexOrThrow(AccountDatabaseHelper.COLUMN_ROLE)) == 1;

        String teacherIdStr = cursor.getString(cursor.getColumnIndexOrThrow(AccountDatabaseHelper.COLUMN_TEACHER_ID));
        String parentIdStr = cursor.getString(cursor.getColumnIndexOrThrow(AccountDatabaseHelper.COLUMN_PARENT_ID));
